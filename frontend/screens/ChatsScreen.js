...
const ChatsScreen = React.memo(function ChatsScreen({ navigation }) {
  const { user } = useAuth();
    ...
    >
      ...
      <View style={styles.conversationContent}>
        <View style={styles.headerRow}>
          ...
          <Text style={[styles.time, { color: 'rgba(0,0,0,0.6)' }]}>
        ...
        <Text 
          style={[
            styles.lastMessage, 
            { color: item.unread_count > 0 ? '#000000' : 'rgba(0,0,0,0.6)' },
    ...
    <View style={{ flex: 1, backgroundColor: isDarkMode ? '#101415' : '#FFFFFF' }}>
      ...
      {conversations.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Icon name="chat-bubble-outline" size={80} color="rgba(255,255,255,0.6)" />

...
const WorkerDetailScreen = React.memo(function WorkerDetailScreen({ route, navigation }) {
  ...
  const handleProceedToPayment = () => {
    ...
    const est = (Number(worker.hourlyRate || 0) * Number(bookingDetails.estimatedHours || 1)).toFixed(2);
    setOfferAmount(prev => (prev && Number(prev) > 0 ? prev : String(est)));
  ...
  const handlePayment = () => {
    ...
    const amount = (Number(offerAmount) || 0).toFixed(2);
    
    if (!upiId.trim()) {
      // Proceed without payment (to be paid after service)
      createBooking('pending_payment');
      return;
    }

    // Generate UPI payment link
    const upiUrl = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(worker.name)}&am=${amount}&cu=INR&tn=${encodeURIComponent('Service Payment - ' + bookingDetails.description)}`;
        ...
        if (supported) {
          ...
          Alert.alert(
            ...
            [
              ...
              { text: 'Proceed', onPress: () => createBooking('pending_payment') }
            ]
          );
        }
      })
      .then(() => {
        // After UPI app opens, immediately create booking as 'paid' for now
        setTimeout(() => createBooking('paid'), 800);
      })
      .catch(() => {
  ...
  try {
    ...
    estimatedPrice: worker.hourlyRate * bookingDetails.estimatedHours,
          ...
          <View style={styles.priceSection}>
            ...
            >
              ...
              <View style={styles.priceInfo}>
                ...
                <Text style={[styles.priceValue, { color: colors.text }]}>${worker.hourlyRate?.toFixed(2) || '0.00'}/hr</Text>
              </View>
            </LinearGradient>
            {worker.minCharge > 0 && (
              <LinearGradient
              ...
              >
                ...
                <View style={styles.priceInfo}>
                  ...
                  <Text style={[styles.priceValue, { color: colors.text }]}>${worker.minCharge?.toFixed(2) || '0.00'}</Text>
                </View>
              </LinearGradient>
            )}
          </View>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            ...
            <View style={styles.modalButtonContainer}>
              <TouchableOpacity 
                style={[styles.modalButton, styles.skipButton, { borderColor: colors.border }]}
                onPress={async () => {
                  // MockPay: create booking as paid and go to review
                  const bookingId = await createBooking('paid');
                  setShowPaymentModal(false);
                  if (bookingId) {
                    navigation.navigate('BookingDetail', { bookingId, showReview: true });
                  } else {
                    Alert.alert('MockPay Success', 'Payment successful!');
                  }
                }}
              >
                <Text style={[styles.skipButtonText, { color: colors.textSecondary }]}>MockPay</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.modalButton, styles.payButton, { backgroundColor: '#5F4BB6' }]}
                onPress={handlePayment}
                disabled={!upiId.trim() || !(Number(offerAmount) > 0)}
              >
